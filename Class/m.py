...
class SpatialAttentionConcatenation(nn.Module):
    def __init__(self, in_channels1, in_channels2):
        super().__init__()
        self.spatial_attention1 = SpatialAttention(in_channels1)
        self.spatial_attention2 = SpatialAttention(in_channels2)
        self.conv1x1 = nn.Conv2d(in_channels1 + in_channels2, in_channels1 + in_channels2, kernel_size=1)

    def forward(self, x1, x2):
        x1_att = self.spatial_attention1(x1)
        x2_att = self.spatial_attention2(x2)
        x_concat = torch.cat((x1_att, x2_att), dim=1)
        return self.conv1x1(x_concat)
...
class SwinTransformer(nn.Module):
    def __init__(self, *, hidden_dim, layers, heads, channels=3, num_classes=1000, head_dim=32, window_size=7,
        ...
        self.stage2 = StageModule(in_channels=hidden_dim, hidden_dimension=hidden_dim * 2, layers=layers[1],
                                  ...
                                  window_size=window_size, relative_pos_embedding=relative_pos_embedding)
        self.stage3 = StageModule(in_channels=288, hidden_dimension=hidden_dim * 4, layers=layers[2],
                                  downscaling_factor=downscaling_factors[2], num_heads=heads[2], head_dim=head_dim,
                                  window_size=window_size, relative_pos_embedding=relative_pos_embedding)
        self.stage4 = StageModule(in_channels=672, hidden_dimension=hidden_dim * 8, layers=layers[3],
                                  downscaling_factor=downscaling_factors[3], num_heads=heads[3], head_dim=head_dim,
                                  window_size=window_size, relative_pos_embedding=relative_pos_embedding)
        self.stage5 = StageModule(in_channels=1440, hidden_dimension=hidden_dim * 16, layers=layers[4],
                                  downscaling_factor=downscaling_factors[4], num_heads=heads[4], head_dim=head_dim,
                                  window_size=window_size, relative_pos_embedding=relative_pos_embedding)

        self.mlp_head = nn.Sequential(
            nn.Linear(1440, num_classes)
        )

        self.attn1 = SpatialAttentionConcatenation(96, 192)
        self.attn2 = SpatialAttentionConcatenation(288, 384)
        self.attn3 = SpatialAttentionConcatenation(672, 768)
    ...
    def forward(self, img):
        ...
        cat1 = self.attn1(F.interpolate(x1, size=(84, 84), mode='nearest'), x2)

        #cat1 = torch.cat((F.interpolate(x1, size=(56, 56), mode='nearest'), x2), dim = 1)

        x3 = self.stage3(cat1)

    
        cat2 = self.attn2(F.interpolate(cat1, size=(42, 42), mode='nearest'), x3)

        #cat2 = torch.cat((F.interpolate(cat1, size=(28, 28), mode='nearest'), x3), dim = 1)

        x4 = self.stage4(cat2)

        cat3 = self.attn3(F.interpolate(cat2, size=(21, 21), mode='nearest'), x4)
        #cat3 = torch.cat((F.interpolate(cat2, size=(14, 14), mode='nearest'), x4), dim = 1)

        #x5 = self.stage5(cat3)
        fl = cat3.mean(dim=[2, 3])
        #fl = torch.flatten(cat3)

        mlp = self.mlp_head(fl)
...
dummy_x = torch.randn(1, 3, 336, 336)
mlp = net(dummy_x)  # (1,3)
...
class SwinTransformer1(nn.Module):
    ...
    def forward(self, img):
        ...
        cat1 = torch.cat((F.interpolate(x1, size=(84, 84), mode='nearest'), x2), dim = 1)
        ...
        cat2 = torch.cat((F.interpolate(cat1, size=(42, 42), mode='nearest'), x3), dim = 1)

        x4 = self.stage4(cat2)

        #cat3 = self.attn3(F.interpolate(cat2, size=(21, 21), mode='nearest'), x4)
        cat3 = torch.cat((F.interpolate(cat2, size=(21, 21), mode='nearest'), x4), dim = 1)
